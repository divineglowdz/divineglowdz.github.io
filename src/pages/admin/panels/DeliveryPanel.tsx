...
export function DeliveryPanel() {
  ...
  const save = async () => { if (!isSupabaseConfigured) return; setSaving(true); await saveDeliveryRates(rates); setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 2000) }
  return <div className="admin-panel-stack"><div className="panel-intro"><div><h2>Tarifs de livraison</h2><p>Personnalisez le prix domicile et bureau pour chaque wilaya.</p></div><button className="button primary small" onClick={save} disabled={saving || !isSupabaseConfigured}>{saved ? <Check /> : <Save />}{saved ? 'Enregistre' : saving ? 'Enregistrement...' : 'Enregistrer'}</button></div>
